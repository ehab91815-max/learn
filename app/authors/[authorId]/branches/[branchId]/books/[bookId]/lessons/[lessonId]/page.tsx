...
import AudioPlayer from "@/components/ui/AudioPlayer";
import YouTubePlayer from "@/components/ui/YouTubePlayer";
import PdfLessonViewer from "@/components/ui/PdfLessonViewer";
...
export default function LessonPage({
  ...
  const handleSavePdfPage = useCallback(
    async (page: number) => {
      if (!uid || !attemptId) return;
  ...
  const pdfUrl = isPdf && media?.r2Key ? r2UrlFromKey(media.r2Key) : "";
    ...
    <main className="mx-auto max-w-3xl p-4 space-y-4">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {!isAudio && !isYoutube && !isPdf ? (
            <p className="text-sm text-muted-foreground">
              نوع محتوى الدرس غير مدعوم حاليًا.
            </p>
          ) : null}
        </CardContent>
      </Card>
